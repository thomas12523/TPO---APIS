...
public interface IDetalleCarritoRepository extends JpaRepository<DetalleCarrito, Integer> {
    ...
    @Query(value = "select dc from DetalleCarrito dc where dc.carrito.carritoId = ?1 and dc.producto.productoId = ?2 and dc.activo = true")
    Optional<DetalleCarrito> findByCarritoIdAndProductoId(int carritoId, int productoId);

    @Query(value = "select dc from DetalleCarrito dc where dc.carrito.carritoId = ?1 and dc.activo = true")
    List<DetalleCarrito> findByCarritoId(int carritoId);

    List<DetalleCarrito> findByActivoTrue();
}
